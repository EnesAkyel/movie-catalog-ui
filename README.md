...
├── genre-navbar/       # Genre filter navbar
...
**Prerequisites:** Node version pinned in `.nvmrc` (use `nvm use`), and [movie-catalog-api](https://github.com/EnesAkyel/movie-catalog-api) running at `http://localhost:8080` (`docker compose up` in that repo).
...
The app runs at `http://localhost:4200`. The API base URL is configured in `src/environments/environment.ts`.
